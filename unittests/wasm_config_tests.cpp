...
struct wasm_config_tester : TESTER {
   ...
   void set_wasm_params(const wasm_config& params) {
      ...
      trx.actions.emplace_back(vector<permission_level>{{SYSTEM_ACCOUNT_NAME,config::active_name}}, SYSTEM_ACCOUNT_NAME, "setwparams"_n,
                               bios_abi_ser.variant_to_binary("setwparams", fc::mutable_variant_object()("cfg", params),
                                                              abi_serializer::create_yield_function( abi_serializer_max_time )));
      trx.actions[0].authorization = vector<permission_level>{{SYSTEM_ACCOUNT_NAME,config::active_name}};
      set_transaction_headers(trx);
      trx.sign(get_private_key(SYSTEM_ACCOUNT_NAME, "active"), control->get_chain_id());
...
BOOST_FIXTURE_TEST_CASE(reset_chain_tests, wasm_config_tester) {
   ...
   // Reset parameters and system contract
   {
      ...
      auto make_setcode = [](const std::vector<uint8_t>& code) {
         return setcode{ SYSTEM_ACCOUNT_NAME, 0, 0, bytes(code.begin(), code.end()) };
      };
      trx.actions.push_back({ { { SYSTEM_ACCOUNT_NAME, config::active_name} }, make_setcode(wast_to_wasm(min_set_parameters_wast)) });
      trx.actions.push_back({ { { SYSTEM_ACCOUNT_NAME, config::active_name} }, SYSTEM_ACCOUNT_NAME, ""_n, fc::raw::pack(genesis_state::default_initial_wasm_configuration) });
      trx.actions.push_back({ { { SYSTEM_ACCOUNT_NAME, config::active_name} }, make_setcode(contracts::eosio_bios_wasm()) });
      set_transaction_headers(trx);
      trx.sign(get_private_key(SYSTEM_ACCOUNT_NAME, "active"), control->get_chain_id());
      push_transaction(trx);
   }

...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(SYSTEM_TOKEN_ACCOUNT_NAME, symbol(SY(4,CUR)), account);
}

const std::string eosio_token = name(SYSTEM_TOKEN_ACCOUNT_NAME).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(SYSTEM_TOKEN_ACCOUNT_NAME);
   ...
   chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("issue"), SYSTEM_TOKEN_ACCOUNT_NAME, fc::mutable_variant_object()
           ("to",       eosio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("transfer"), SYSTEM_TOKEN_ACCOUNT_NAME, fc::mutable_variant_object()
       ("from", eosio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, SYSTEM_TOKEN_ACCOUNT_NAME);
   ...
   trace = chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("transfer"), "tester"_n, fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, SYSTEM_TOKEN_ACCOUNT_NAME);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, "tester"_n);
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   name sname(SYSTEM_TOKEN_ACCOUNT_NAME);
   // link auth
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ("account", "tester")
   ...
   trace = chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("transfer"), SYSTEM_TOKEN_ACCOUNT_NAME, fc::mutable_variant_object()
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(SYSTEM_TOKEN_ACCOUNT_NAME);
   auto abi = acnt.get_abi();
   chain::abi_serializer abis(std::move(abi), abi_serializer::create_yield_function( chain.abi_serializer_max_time ));
   const auto a = chain.control->db().get<account_object,by_name>(SYSTEM_TOKEN_ACCOUNT_NAME).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = SYSTEM_TOKEN_ACCOUNT_NAME;
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("transfer"), vector<permission_level>{{"tester"_n, "first"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("transfer"), vector<permission_level>{{"tester"_n, "second"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("transfer"), vector<permission_level>{{"tester"_n, config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
...
BOOST_AUTO_TEST_CASE( max_transaction_delay_execute ) { try {
   ...
   chain.push_action(SYSTEM_TOKEN_ACCOUNT_NAME, name("issue"), "tester"_n, fc::mutable_variant_object()
           ("to",       "tester")
           ("quantity", "100.0000 CUR")
           ("memo", "for stuff")
   );

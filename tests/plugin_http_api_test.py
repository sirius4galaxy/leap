...
from TestHarness import Account, Node, TestHelper, Utils, WalletMgr, ReturnType, system_config
...
class PluginHttpTest(unittest.TestCase):
    ...
    def startEnv(self) :
        ...
        start_nodeos_cmd = (f"%s -e -p {system_config.SYSTEM_ACCOUNT_NAME} %s %s ") % (Utils.EosServerPath, nodeos_plugins, nodeos_flags)
    ...
    def activateAllBuiltinProtocolFeatures(self):
        ...
        eosioAccount = Account(f"{system_config.SYSTEM_ACCOUNT_NAME}")
    ...
    def test_ChainApi(self) :
        ...
        payload = {"code":f"{system_config.SYSTEM_TOKEN_ACCOUNT_NAME}", "account":"unknown"}
        ret_json = self.nodeos.processUrllibRequest(resource, command, payload)
        self.assertEqual(ret_json["code"], 500)
        ...
        payload = {"code":f"{system_config.SYSTEM_TOKEN_ACCOUNT_NAME}","symbol":"SYS"}
        ret_json = self.nodeos.processUrllibRequest(resource, command, payload)
        self.assertEqual(ret_json["code"], 500)
        ...
        payload = {"expiration":"2020-08-01T07:15:49",
                   ...
                   "delay_sec":0,
                   "context_free_actions":[],
                   "actions":[{"account":f"{system_config.SYSTEM_TOKEN_ACCOUNT_NAME}","name": "transfer","authorization": [{"actor": "han","permission": "active"}],"data": "000000000000a6690000000000ea305501000000000000000453595300000000016d"}],
                   "transaction_extensions": [],
                   "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                   "context_free_data": []}
        ...
        payload_hex = {"expiration":"2020-08-01T07:15:49",
                ...
                "delay_sec":0,
                "context_free_actions":[],
                "actions":[{"account":f"{system_config.SYSTEM_TOKEN_ACCOUNT_NAME}","name": "transfer","authorization": [{"actor": "han","permission": "active"}],
